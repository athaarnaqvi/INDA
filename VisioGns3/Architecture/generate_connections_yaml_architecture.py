...
# Building_description → Architecture → VisioGns3
# ─────────────────────────────────────────────
...
def generate_ansible_playbook(ip, port, project_name, connections):
    playbook = f"""---
- name: Create Architecture-based connections in GNS3
  hosts: localhost
  gather_facts: no
...
if __name__ == "__main__":
    ...
    require_file(CONNECTIONS_FILE, "Architecture connections JSON")
    require_file(VSDX_PATH_FILE , "Architecture project name")
    ...
    print("[SUCCESS] Architecture Connections playbook generated:")
